...
@section('style')
    ...
    unicode-bidi: bidi-override; /* Override the default direction */
}
...
.rating label {
    ...
    color: #aaa; /* Default color of the stars */
    cursor: pointer; /* Change cursor to pointer on hover */
}

.rating input:checked ~ label {
    color: #ffcc00; /* Change color of selected stars */
}

.rating label:hover,
.rating label:hover ~ label {
    color: #ffcc00; /* Change color of hovered stars and those before it */
}
    </style>
...
@section('content')
    <div class="container" style="margin-top: 50px">
        <div class="row">
                ...
                <div class="col-md-8">
                    <form id="comment-form" action="{{ route('user.comment.store') }}" method="POST">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" name="comments" id="comments" placeholder="Give feedback" required>
                            <div class="input-group-append">
                                <button type="submit" class="btn btn-primary btn-block" id="give-feedback"><i class="fa fa-send"></i></button>
                            </div>
                        </div>
                    </form>
                </div>
                <div class="col-md-4">
                    <button id="show-more-comments" class="btn btn-outline-primary btn-block" style="height: 60px;">Show more reviews</button>
                </div>
            </div>
            ...
            <div class="form-group row">
                <div class="col-md-12">
                    <button id="show-more-comments" class="btn btn-outline-primary btn-block">Show more reviews</button>
                </div>
            </div>
            @endif
            @if($errors->any())
...
$(document).ready(function() {
    ...
    function appendComments(comments) {
        ...
        $.each(comments, function(index, comment) {
            ...
            commentsContainer.append('<div class="rating">' + getStarRating(comment.rating) + '</div>');
            commentsContainer.append('<div class="comment">' + comment.comments + '</div>');
            commentsContainer.append('<div class="comment-date" style="color: grey;">' + comment.formatted_created_at + '</div><br>');
        });
    }
